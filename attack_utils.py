...
import numpy as np
import sys
# sys.path.append('/home/mengjie/PycharmProjects/AttackSiamTracker-main')
sys.path.append('/home/mengjie/PycharmProjects/AttackSiamTracker-main/pysot')
...
# get loss when testing

def get_losses(img_tensor,zhanbi,AdA,search_sz=(287,287)):
    img_tensor_1 = normalize(img_tensor)
    '''step2: pass to G'''
    with torch.no_grad():
        AdA.search_clean255 = img_tensor
        AdA.search_clean1 = img_tensor_1
        AdA.num_search = img_tensor.size(0)
        AdA.zhanbi=zhanbi

        loss = AdA.get_loss(search_sz)
        # AdA.forward(search_sz)
...
# get
def get_clean_1(img_tensor,zhanbi,AdA,search_sz=(287,287)):
    '''adversarial attack to search region'''
    '''input: pytorch tensor(0,255) ---> output: pytorch tensor(0,255)'''
    '''step1: Normalization'''
    img_tensor = normalize(img_tensor)
    ...
    img_search_clean1 = AdA.template
    return img_search_clean1
