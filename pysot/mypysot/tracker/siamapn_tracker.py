...
from pysot.mypysot.tracker.base_tracker import SiameseTracker
...
class SiamAPNTracker(SiameseTracker):
    ...
    def generate_anchor(self):
        ...
        x = np.tile((cfg.ANCHOR.STRIDE * (np.linspace(0, size - 1, size)) + cfg.TRAIN.MOV) - cfg.TRAIN.SEARCH_SIZE // 2,
                    size).reshape(-1)
        y = np.tile((cfg.ANCHOR.STRIDE * (np.linspace(0, size - 1, size)) + cfg.TRAIN.MOV).reshape(-1,
                                                                                                   1) - cfg.TRAIN.SEARCH_SIZE // 2,
                    size).reshape(-1)
    ...
    def init(self, img, bbox):
        # get the template: ground truth box in the first frame
        """
        args:
        ...
        # get crop
        z_crop, context_2, contex_1, org_patch_size = self.get_subwindow(img, self.center_pos,
    ...
    def get_z_crop(self, img, bbox):
        ...
        z_crop, context_2, contex_1, org_patch_size = self.get_subwindow(img, self.center_pos,
                                    cfg.TRACK.EXEMPLAR_SIZE,
                                    s_z, self.channel_average)
        return z_crop, context_2, contex_1, org_patch_size
    ...
    def get_x_crop(self, img):
        ...
        x_crop, context_2, contex_1, org_patch_size = self.get_subwindow(img, self.center_pos,
                                    cfg.TRACK.INSTANCE_SIZE,
                                    round(s_x), self.channel_average)

        return x_crop, scale_z, context_2, contex_1, org_patch_size
    ...
    def save_adv_img(self, tensor_adv, save_path, frame_id):

        ## adv x_crop
        img_adv = tensor2img(tensor_adv)
        cv2.imwrite(os.path.join(save_path, '%04d_adv.jpg' % frame_id), img_adv)

    def my_save_img(self, tensor_adv, tensor_filter, save_path, frame_id):
        # tensor_adv = np.expand_dims(tensor_adv, axis=0)
        # tensor_filter = np.expand_dims(tensor_filter, axis=0)
        # tensor_adv = torch.from_numpy(tensor_adv)
        # tensor_filter = torch.from_numpy(tensor_filter)
        # tensor_adv = tensor_adv.clip(0, 255).astype(np.uint8)
        # tensor_filter = tensor_filter.clip(0, 255).astype(np.uint8)
        ## clean x_crop

        img_adv = tensor_adv.clip(0, 255).astype(np.uint8)

        cv2.imwrite(os.path.join(save_path, '%04d_adv.jpg' % frame_id), img_adv)
        ## adv x_crop
        img_filter = tensor_filter.clip(0, 255).astype(np.uint8)
        cv2.imwrite(os.path.join(save_path, '%04d_filter.jpg' % frame_id), img_filter)
        ## diff
        tensor_diff = (tensor_adv - tensor_filter) * 10
        # print(torch.mean(torch.abs(tensor_diff)))
        tensor_diff += 127
        img_diff = tensor_diff.clip(0, 255).astype(np.uint8)
        cv2.imwrite(os.path.join(save_path, '%04d_diff.jpg' % frame_id), img_diff)
    ...
    def init(self, img, bbox):
        if img.shape[0] != 127:
            z_crop, context_2, contex_1, org_patch_size = self.get_z_crop(img, bbox)
        else:
            z_crop = img
        self.template = z_crop
        self.model.template(z_crop)

    def init_adv(self, img, bbox, zhanbi, GAN):
        z_crop, context_2, contex_1, org_patch_size= self.get_z_crop(img, bbox)
        z_crop_adv = adv_attack_search(z_crop, zhanbi, GAN, search_sz=(127, 127))
        self.template = z_crop_adv
    ...
    def track_adv(self, img, zhanbi, GAN, save_path=None, frame_id=None):
        x_crop, scale_z, context_2, contex_1,  org_patch_size= self.get_x_crop(img)
        '''Adversarial Attack'''
        zhanbi = zhanbi
        # print(zhanbi)
        ...
        return output_dict, x_crop_adv, context_2, contex_1, org_patch_size, x_crop

    #############################

    def check_loss(self, img, zhanbi, GAN, save_path=None, frame_id=None):
        x_crop, scale_z, context_2, contex_1, org_patch_size = self.get_x_crop(img)
        '''Adversarial Attack'''
        zhanbi = zhanbi
        # print(zhanbi)
        # print("x_crop:",x_crop.shape)
        loss = get_losses(x_crop, zhanbi, GAN)

        return loss

    #############################
    def track_get_clean(self, img, zhanbi, GAN, save_path=None, frame_id=None):
        x_crop, scale_z, context_2, contex_1, org_patch_size = self.get_x_crop(img)
        '''Adversarial Attack'''
        zhanbi = zhanbi
        # print(zhanbi)
        # print("x_crop:",x_crop.shape)
        GAN.template = self.template
        x_crop_adv = get_clean_1(x_crop, zhanbi, GAN)
    ...
    # def track(self, img, x_crop): delete the x_crop argument
    def track_base(self, img):
        """
        args:
            img(np.ndarray): BGR image
        return:
        ...
        # replace self.model.track(x_crop) with self.model.track_base(x_crop)
        outputs = self.model.track(x_crop)
        self.anchors = self.generate_anchor()
        score1 = self._convert_score(outputs['cls']) * cfg.TRACK.w1
        score2 = self._convert_score(outputs['cls2']) * cfg.TRACK.w2
